...
Если в теле запроса передаются параметры, то они должны быть закодированы в формате `application/x-www-form-urlencoded` и с соответствующим заголовком:
...
carParams | Массив параметров транспорта, может иметь следующие поля: <ul><li>`is_isotherm` - признак изотермы, 0 или 1,</li><li>`is_refrigerator` - признак рифрижератора, 0 или 1</li></ul>_опциональный параметр_|
